...
os.Args的第一个元素是os.Args\[0\]， 就是命令本身的名字。os.Args剩下的其他元素就是执行程序时传入的参数。表达式s\[m:n\]表示从第m个元素到第n-1个元素组成的一个新slice。如果m或是n省略掉的话，表达式就表示从0开始或是直到len\(s\)的slice。所以，我们可以把想要的slice简记为os.Args\[1:\]。
